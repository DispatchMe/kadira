...
// msg._kadiraInfo is {session: <session_id>, userId: <user id>}
PubsubModel.prototype._trackSub = function(session, msg) {
  ...
  Kadira.logger.info('subscribed', {
    user_id: session.userId,
    session_id: session.id,
    subscription: msg.name,
...
// session should have properties <id> and <userId>
PubsubModel.prototype._trackUnsub = function(session, sub) {
  ...
  Kadira.logger.info('unsubscribed', {
    user_id: session.userId,
    session_id: session.id,
    subscription: sub._name,
    publication: publication,
  });
...
PubsubModel.prototype._trackReady = function(session, sub, trace) {
  ...
  if(subscriptionState && !subscriptionState.readyTracked) {
    readyTime = timestamp - subscriptionState.startTime;
    metrics.resTime += readyTime;
    Kadira.datadogClient
    subscriptionState.readyTracked = true;
  }

  Kadira.logger.info('subscription_ready', {
    user_id: session.userId,
    session_id: session.id,
    subscription: sub._name,
    publication: publication,
  ...
  if (readyTime) {
    Kadira.datadogClient.timing('kadira.pubsub.ready_time', readyTime, Kadira.timingSampleRate, ['publication:' + publication]);
  }
...
PubsubModel.prototype._trackError = function(session, sub, trace) {
  ...
  Kadira.logger.error('subscription_error', {
    user_id: session.userId,
    session_id: session.id,
    subscription: sub._name,
    publication: publication,
...
PubsubModel.prototype.sendPeriodicMetrics = function(buildDetailInfo) {
  ...
  Object.keys(activeSubs).forEach(funcption(publicationName) {
    Kadira.datadogClient.gauge('kadira.pubsub.active_subscriptions', activeSubs[publicationName], ['publication:' + publicationName]);
  });

  Object.keys(activeDocs).forEach(function(publicationName) {
    Kadira.datadogClient.gauge('kadira.pubsub.active_documents', activeDocs[publicationName], ['publication:' + publicationName]);
  });
};
...
PubsubModel.prototype.trackCreatedObserver = function(info) {
  ...
  Kadira.datadogClient.increment('kadira.pubsub.created_observers', 1, ['publication:' + publicationName]);
}
...
PubsubModel.prototype.trackDeletedObserver = function(info) {
  ...
  Kadira.datadogClient.increment('kadira.pubsub.deleted_observers', 1, ['publication:' + publicationName]);
...
PubsubModel.prototype.trackDocumentChanges = function(info, op) {
  ...
  var publication = this._getMetrics(timestamp, publicationName);
  if(op.op === "d") {
    Kadira.datadogClient.increment('kadira.pubsub.oplog_deleted_documents', 1, ['publication:' + publicationName]);
    publication.oplogDeletedDocuments++;
  } else if(op.op === "i") {
    Kadira.datadogClient.increment('kadira.pubsub.oplog_inserted_documents', 1, ['publication:' + publicationName]);
    publication.oplogInsertedDocuments++;
  } else if(op.op === "u") {
    Kadira.datadogClient.increment('kadira.pubsub.oplog_updated_documents', 1, ['publication:' + publicationName]);
...
PubsubModel.prototype.trackPolledDocuments = function(info, count) {
  ...
  Kadira.datadogClient.increment('kadira.pubsub.polled_documents', count, ['publication:' + publicationName]);
}
...
PubsubModel.prototype.trackLiveUpdates = function(info, type, count) {
  ...
  if(type === "_addPublished") {
    Kadira.datadogClient.increment('kadira.pubsub.live_added_documents', count, ['publication:' + publicationName]);
    publication.liveAddedDocuments += count;
  } else if(type === "_removePublished") {
    Kadira.datadogClient.increment('kadira.pubsub.live_removed_documents', count, ['publication:' + publicationName]);
    publication.liveRemovedDocuments += count;
  } else if(type === "_changePublished") {
    Kadira.datadogClient.increment('kadira.pubsub.live_changed_documents', count, ['publication:' + publicationName]);
    publication.liveChangedDocuments += count;
  } else if(type === "_initialAdds") {
    Kadira.datadogClient.increment('kadira.pubsub.initially_added_documents', count, ['publication:' + publicationName]);
