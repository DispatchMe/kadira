...
var DogStatsDClient = Npm.require('node-dogstatsd').StatsD;
var KadiraCore = Npm.require('kadira-core').Kadira;
var Logstar = Npm.require('logstar').Logger;
...
Kadira.connect = function(serviceName, options) {
  options = options || {};
  ...
  options.datadogAgentHost = options.datadogAgentHost || 'datadog';
  options.datadogAgentPort = options.datadogAgentPort || '8125';
  options.logThreshold = options.logThreshold || 'error';
  ...
  Kadira.logger = new Logstar({
    logLevel: options.logThreshold,
    loggly: {
      token: options.logglyToken,
      subdomain: options.logglySubdomain,
      tags: options.logglyTags,
    },
  });
  ...
  var tags = [];
  if (process.env.ENV) {
    tags.push(`environment:${process.env.ENV}`);
  }
  ...
  Kadira.datadogClient = new DogStatsDClient(options.datadogAgentHost, options.datadogAgentPort, null, {
    global_tags: tags,
  });
  ...
  Kadira._schedulePeriodicMetrics();
  ...
  // Add method for the client to track errors
  Meteor.methods({
    'kadira/send': function(parameters) {
      ...
      switch (endpoint) {
        case '/errors':
          (payload.errors || []).forEach(function(err) {
            Kadira.models.error.trackClientError(connection, err);
          });
          break;
      }
    }
...
Kadira._schedulePeriodicMetrics = function () {
  setTimeout(function () {
    Kadira._sendPeriodicMetrics(Kadira._schedulePeriodicMetrics);
  }, Kadira.options.payloadTimeout);
}

function metricName(metric) {
  return 'kadira.' + metric;
}

var ignoreKey = '<not-a-method-or-a-pub>';

Kadira._sendPeriodicMetrics = function (callback) {
  new Fibers(function() {
    Kadira.models.methods.sendPeriodicMetrics();
    Kadira.models.system.sendPeriodicMetrics();
    Kadira.models.pubsub.sendPeriodicMetrics();
    callback();
    return;
  }).run();
}
